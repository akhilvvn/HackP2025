...
st.title("Search Images with Text (CLIP)")
...
if query:
    ...
    top_indices = valid_indices[:max_results]
    ...
    if top_indices:
        ...
        for rank, idx in enumerate(top_indices, start=1):
            with cols[rank - 1]:
                st.image(image_paths[idx], caption=f"Rank {rank} | Score {sims[idx]:.3f}", use_column_width=True)
            results.append({
                "query": query,
                "rank": rank,
